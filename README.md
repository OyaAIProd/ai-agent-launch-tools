...
```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.8 mcp-trust-check --server "candidate MCP server" --workflow "one AI workflow that can read docs and call approved tools"
```

Review a redacted MCP client config before installing or approving servers:

```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.8 mcp-config-risk --file claude_desktop_config.redacted.json
```
...
```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.8 mcp-config-risk --file claude_desktop_config.redacted.json --json
```

JSON output:

```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.8 mcp-trust-check --server "candidate MCP server" --json
```

Generate an MCP permission matrix from a `tools/list` JSON result:

```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.8 mcp-permission-matrix --file tools-list.json --server "candidate MCP server"
```
...
```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.8 mcp-permission-matrix --file tools-list.json --json
```

Compare against a prior JSON matrix snapshot:

```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.8 mcp-permission-matrix --file tools-list.json --baseline previous-matrix.json
```

Print only a Codex `config.toml` review snippet:

```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.8 mcp-permission-matrix --file tools-list.json --server "candidate MCP server" --codex-config
```

Run the public launch-surface scanner from GitHub:

```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.8 public-surface-scan https://example.com
```
...
```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.8 public-surface-scan https://example.com --json
```

Markdown report:

```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.8 public-surface-scan https://example.com --markdown
```
