...
It now includes a small `tools/list` importer that turns MCP tool metadata into an allow / ask / deny permission matrix with a snapshot digest for re-reviewing changed tools, without invoking any tools.
...
```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.6 mcp-trust-check --server "candidate MCP server" --workflow "one AI workflow that can read docs and call approved tools"
```

JSON output:

```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.6 mcp-trust-check --server "candidate MCP server" --json
```

Generate an MCP permission matrix from a `tools/list` JSON result:

```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.6 mcp-permission-matrix --file tools-list.json --server "candidate MCP server"
```
...
```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.6 mcp-permission-matrix --file tools-list.json --json
```

Compare against a prior JSON matrix snapshot:

```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.6 mcp-permission-matrix --file tools-list.json --baseline previous-matrix.json
```

Run the public launch-surface scanner from GitHub:

```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.6 public-surface-scan https://example.com
```
...
```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.6 public-surface-scan https://example.com --json
```

Markdown report:

```bash
npx --package github:kayalopez/ai-agent-launch-tools#v0.1.6 public-surface-scan https://example.com --markdown
```
...
The MCP trust planner blocks obvious secret-like inputs and prints a non-sensitive launch checklist. The MCP permission matrix importer reads metadata only, never invokes MCP tools, and can compare a new metadata snapshot against a prior JSON matrix so added, removed, or changed tools re-enter review. These tools are for review planning only, not approval automation or a guarantee that a server is safe.
